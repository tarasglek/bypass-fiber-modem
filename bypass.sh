...
ip link set br0 up
ip link del br0.0
ip link add link br0 name br0.0 type macvlan
ip link set wan0 up
...
ebtables -t filter -A FORWARD -i bridge0 -p 802_1Q --vlan-encap 0x888e -j ACCEPT
ebtables -t filter -A FORWARD -i bridge0 -p 802_1Q -j DROP
ebtables -t filter -A FORWARD -o bridge0 -p 802_1Q --vlan-encap 0x888e -j ACCEPT
ebtables -t filter -A FORWARD -o bridge0 -p 802_1Q -j DROP
cd `dirname $0` && ./.venv/bin/python steal_dhcp.py wan0 br0.0 |sh